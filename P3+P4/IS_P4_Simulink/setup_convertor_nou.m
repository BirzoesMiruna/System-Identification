...
tr=0.02*4 %t90-t10  %dublam timpul de urcare inmultind cu 4 ca sa vedem mai bine oscilatiile pe iesire, asta dupa ce avem graficul facut( din grafic din poza 1.02-1.00) -tr reprezinta timpul de urcare
...
%plot(input_LUT_dSPACE(:,1),input_LUT_dSPACE(:,2))
%shg

%% Data acquisition (use t, u, y to perform system identification)
out = sim("convertor_R2022b_nou.slx");
...
u_id=u_id - mean(u_id);
y_id=y(i1:N:i2);
y_id=y_id - mean(y_id); % astea facem daca nu merg una dintre modelele parametrice
...
%%
%Testam modelul Armax
model_armax=armax(dat_id,[2,2,1,1]); % avem 4 parametrri de structura nA nB nD - care i i am gasit mai sus + nC care se ia prin incercari
