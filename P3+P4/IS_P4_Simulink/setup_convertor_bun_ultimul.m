...
t1=0.2; 
N=5;
tr=0.04;%timpul primei oscilatii % am imultit cu 4 ca sa vedem si ceva peturbatii
p= round(tr/N/Ts)
...
out = sim("convertor_R2022b_cu_bun.slx");
...
%% bun
model_armax=armax(dat_id,[2,2,3,4])                  % (dat_id,[nA nB nC nd) avem 4 parametrri de structura nA nB nD - care i i am gasit mai sus + nC care se ia prin incercari (nA,nB,nC,nD)
...
%%
%Testamo modelul OE
model_oe=oe(dat_id,[2,2,7])
 figure,resid(model_oe,dat_vd)
 figure,compare(model_oe,dat_vd)
%%
% Testam modelul Box Jenkins
%model_bj=bj(dat_id,[2 2 1 4 3]);
model_bj=bj(dat_id,[3 3 2 8 1]);
modelbj=pem(dat_id,model_bj)
 figure,resid(modelbj,dat_vd);
 figure,compare(modelbj,dat_vd);
 %% bun
 % 1. Eliminarea tendințelor/mediilor
ze = detrend(dat_id);
zv = detrend(dat_vd);
% 2. Identificare cu o structură robustă
% nf mai mare ajută la eliminarea corelației din XCorr
model_bj_rafinat = bj(ze, [3 3 2 8 1]); 

% 3. Verificare
figure; resid(model_bj_rafinat, zv);
figure; compare(model_bj_rafinat, zv);
...
%%
figure
step(model_bj_rafinat)
title('Raspunsul indicial (la treapta)')
grid on
z = zero(model_bj_rafinat)
p = pole(model_bj_rafinat)
%%
figure
step(model_n4sid)
title('Raspunsul indicial (la treapta)')
